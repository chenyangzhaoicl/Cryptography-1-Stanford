...
# List of ciphertexts, with the last one being the target ciphertext
CIPHERTEXTS = [
...
def possible_xor_results(s: str) -> defaultdict[int, list[tuple[int, int]]]:
    """ Generate a dictionary mapping each possible XOR result to the character pairs that produce it. """
...
def main() -> None:
    # Generate possible XOR pairs for ASCII letters and space
    possible_xor_pairs = possible_xor_results(string.ascii_letters + " ")

    # Initialize a list to count potential key values for each byte position
    ...
    for c1, c2 in combinations(CIPHERTEXTS, 2):
        for i, (b1, b2) in enumerate(zip(c1, c2)):
            ...
            for x, y in possible_xor_pairs[b1 ^ b2]:
                possible_cipher_chars.update({b1 ^ x, b1 ^ y})
            for c in possible_cipher_chars:
                possible_keys[i][c] += 1

    # Guess the most likely key for each byte position
    key_guess = [max(keys, key=keys.get) for keys in possible_keys]

    # Decrypt each ciphertext using the guessed key and print the results
    for i, cs in enumerate(CIPHERTEXTS):
        plaintext_guess = ''.join(chr(x ^ k) if k is not None else '?' for x, k in zip(cs, key_guess))
        print(f"{i+1:2} | {plaintext_guess}")

if __name__ == "__main__":
    main()


#When two ciphertexts encrypted with the same key are XORed together, the key cancels out, revealing the XOR of the plaintexts!!!